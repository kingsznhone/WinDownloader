...
using POC.Models;
using static POC.Services.EsdToIsoOutputWriter;
using static POC.Services.EsdToIsoPipelinePlanning;
using static POC.Services.EsdToIsoProgressFactory;
...
public sealed class EsdToIsoPipelineService : IEsdToIsoPipelineService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var installTarget = GetInstallTarget(request.InstallFormat, sourcesDirectory);
            ...
            var installImagePaths = new List<string> { installTarget.Path };
            ...
            {
                var esdBaseName = Path.GetFileNameWithoutExtension(request.SourceEsdPath);
                var esdDirectory = Path.GetDirectoryName(request.SourceEsdPath) ?? Environment.CurrentDirectory;
                var isoPath = Path.Combine(esdDirectory, $"{esdBaseName}.iso");
        ...
        CancellationToken cancellationToken)
    {
        ...
        return await _isoCreationService.CreateIsoAsync(
            request,
