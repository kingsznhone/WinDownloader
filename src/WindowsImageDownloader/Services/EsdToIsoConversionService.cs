...
public sealed class EsdToIsoConversionService : IEsdToIsoConversionService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            session.Publish(EsdToIsoTaskState.Running, EsdToIsoStage.BuildingInstallImage, 0.50, session.InstallEsdPath, force: true);
    ...
    private static void ValidateRequest(EsdToIsoRequest request)
    {
        ...
        if (string.IsNullOrWhiteSpace(request.StagingRoot))
        {
            throw new ArgumentException("Staging root directory cannot be null or empty.", nameof(request));
        }
    }
