...
public sealed class UpdateCatalogService : IUpdateCatalogService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
                ...
                !await VerifySha256Async(cabPath, expectedDigest, cancellationToken))
            {
                ...
                {
                    throw new InvalidOperationException("products.cab SHA-256 check failed.");
                }
            }
    ...
    private async Task<(string Url, string Digest)> SearchCatalogAsync(CancellationToken cancellationToken)
    {
        ...
        throw new InvalidOperationException("No available products.cab download URL found in the Windows Update response.");
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidOperationException($"expand.exe failed with exit code {process.ExitCode}. stdout: {stdOut} stderr: {stdErr}");
        }

        if (!File.Exists(outputPath))
        {
            throw new FileNotFoundException(
                $"expand.exe did not generate products.xml (exit code {process.ExitCode}). stdout: {stdOut} stderr: {stdErr}",
                outputPath);
        }
    }
