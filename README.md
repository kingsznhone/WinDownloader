...
- 主项目 `src/WinDownloader`：WinUI 图形应用，包含 ESD 下载流水线和 ISO 转换入口。
- WIM/ISO 共享库：`src/WinDownloader.Wim` 封装 ManagedWimLib，`src/WinDownloader.Iso` 封装 ISO 打包和 oscdimg 后端；主应用内的 `EsdToIsoConversionService` 负责 ESD→ISO 流水线。
...
从 [docs/WORKFLOW.md](docs/WORKFLOW.md) 开始阅读；总体结构见 [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)。ESD 下载见 [docs/MODULE_DOWNLOAD.md](docs/MODULE_DOWNLOAD.md)，ISO 转换调度见 [docs/MODULE_CONVERSION.md](docs/MODULE_CONVERSION.md)。本地化资源、PRI 行为和语言切换规则见 [docs/MODULE_LOCALIZATION.md](docs/MODULE_LOCALIZATION.md)。
