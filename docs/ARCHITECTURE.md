...
ISO 转换已经是主项目能力：ManagedWimLib 负责读取、提取和导出 ESD/WIM 映像，随输出复制的 `Oscdimg` 工具负责最终 ISO 创建。转换任务不写入 SQLite，不扩展下载任务 `TaskState`；UI 通过独立 `EsdToIsoTaskSnapshot` 显示转换状态。
...
| 接口 | `Interfaces/IWimProcessingService.cs` | ManagedWimLib 操作契约 |
| 接口 | `Interfaces/IIsoCreationService.cs` | ISO 创建后端契约 |
...
| 服务 | `Services/EsdToIsoConversionService.cs` | ESD 到 ISO 产品层转换流水线 |
| 服务 | `Services/WimProcessingService.cs` | ManagedWimLib 单例包装，串行化 WIM 操作 |
| 服务 | `Services/OscdimgIsoCreationService.cs` | 调用 `oscdimg.exe` 创建 ISO，并解析进度 |
...
DownloadTaskItemViewModel.ConvertToIsoAsync
  ...
  → WimProcessingService.ExportImagesAsync image 4..n 到 install.esd
...
服务职责整体仍然清晰：`DownloadService` 只包装 Downloader，`EsdDownloadPipeline` 只负责下载和校验，`EsdToIsoConversionService` 只负责转换流水线，`WimProcessingService` 和 `OscdimgIsoCreationService` 分别封装 WIM/ESD 原语和 ISO 后端。
...
| EsdToIsoConversionService | IWimProcessingService, IIsoCreationService | TaskOrchestratorService |
| WimProcessingService | ManagedWimLib | EsdToIsoConversionService |
| OscdimgIsoCreationService | bundled Oscdimg, process execution | EsdToIsoConversionService |
...
`src/POC` 是控制台验证和对照宿主。它保留与主项目相近的 WIM/ISO 模型和服务，用于验证转换流水线、进度映射、压缩参数和 oscdimg 输出解析；主项目已经拥有正式 WinUI 入口和服务注册。POC 行为可以更偏诊断，不能自动视为主应用产品承诺。
